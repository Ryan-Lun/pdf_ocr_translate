...
| `TRANSLATION_GLOSSARY_SOURCE` | 選填 | `sql` | Glossary 來源設定。正式環境應維持 SQL-first 的 `sql`；只有 transition rollback 或短期比對問題時才暫時設為 `json`。 |
| `GLOSSARY_CONTEXT_ARTIFACT_ENABLED` | 選填 | `0` | Debug 用開關。預設不要開啟；設為 `true` 時，每個翻譯 job 會輸出包含完整部門詞彙庫 snapshot 的 `glossary_context.json`，只建議排查詞彙庫載入問題時暫時啟用。 |
